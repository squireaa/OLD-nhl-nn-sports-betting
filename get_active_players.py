...
for i in range(5):
    ...
    for i in range(len(shifts)):
        player_code = "{2} {0} {1}".format(shifts['first_name'][i], shifts['last_name'][i], shifts['team_abbreviation'][i])
    ...
    if str(teams_list[i][1]) == str(team1_letters):
        ...
        df.at[row_index, 'player_list'] = team2_list
        row_index += 1
        df.at[row_index+1, 'my_id'] = "{0}-{1}".format(teams_list[i][1], teams_list[i][0])
        df.at[row_index+1, 'player_list'] = team1_list
        row_index += 1
